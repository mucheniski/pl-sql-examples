...
SET VERIFY OFF
DECLARE
  TYPE Numero_Table_Type IS VARRAY (10) OF INTEGER(2); -- A diferença entre o nested table é que precisa ser definido o tamanho máximo na declaração IS VARRAY (10).
  Numero_table numero_table_type := numero_table_type();
BEGIN
  -- Armazena números de 1 a 10 em um Varray
  FOR i IN 1..10 LOOP
    Numero_Table.extend; -- Aloca uma ocorrência ao array com extend
    Numero_Table(i) := i;
  END LOOP;
