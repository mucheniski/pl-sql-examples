...
--
DECLARE
  TYPE Numero_Table_Type IS TABLE OF INTEGER(2); -- Sem index by é um Nested Table
  Numero_table numero_table_type := numero_table_type(); -- Inicializado
BEGIN
  -- Armazena números de 1 a 10 em um Nested Table
  FOR i IN 1..10
  LOOP
    Numero_Table.extend; -- Alocada a ocorrência, no Nested Table precisa ser feito assim antes de atribuir
    Numero_Table(i) := i;
  END LOOP;
  --
