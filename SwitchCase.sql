...
DECLARE
   ...
   vDepartment_id  employees.department_id%type := &pdepartment_id; -- O & indica que é uma variável de substituição
BEGIN
   CASE  vDepartment_id 
   WHEN  80 
   THEN 
...
BEGIN
   CASE  
   WHEN  vDepartment_id = 80 -- Nesse caso como a comparação é no when, pode ser colocado qualquer outro valor aqui ex vDepartment_id = 80 AND ...
   THEN 
        vpercentual := 10; -- Sales
